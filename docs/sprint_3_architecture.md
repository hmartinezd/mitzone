...
## Encounter eligibility and blocking

Relevance runs only after eligibility/safety and never creates, validates, or blocks an encounter. `EncounterRelevance` is a separate, storage-neutral result containing the encounter ID, a bounded 0.0–1.0 score, and structured signal results. Initial centralized weights cover shared context, capped overlap, interests, languages, connection goals, and profile completeness. A stable encounter-ID tie-breaker makes ordering deterministic.

Signals use public profile/context data only. Interests and languages compare trimmed, case-insensitive values. Equal known goals are positive; different goals are neutral. Missing optional data is unknown/neutral and never excludes a user. Overlap reaches its maximum contribution at 30 minutes. A future optional `personalityCompatibility` signal may be added, but cannot be required or safety-related. Scores remain internal for now.

`EncounterEligibilityPolicy` is the small storage-neutral decision boundary: distinct users, a still-valid overlap, and no directional block in either direction are required for actionability. Safety-sensitive lookup failures fail closed as `interactionUnavailable`. Production blocks are directional rows owned by the authenticated blocker (`0004_blocks_and_eligibility.sql`), while encounter visibility uses symmetric restriction through RLS. The `check_interaction_eligibility` RPC is the backend boundary for future interaction commands; client filtering is defense in depth. Unblocking does not restore prior interactions.
