...
class SupabaseAuthRepository implements AuthRepository {
  ...
  @override Future<AuthSession> signIn({required String email, required String password}) async { try { final session = _map((await client.auth.signInWithPassword(email: email, password: password)).session); if (session == null) throw const DomainError(DomainErrorCode.unauthorized, 'Authentication did not return a session'); return session; } on DomainError { rethrow; } catch (_) { throw const DomainError(DomainErrorCode.unauthorized, 'Authentication failed'); } }
