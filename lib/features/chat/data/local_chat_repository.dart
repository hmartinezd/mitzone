...
class LocalChatRepository implements ChatRepository {
  ...
  @override Future<Message> sendMessage({required String conversationId,required String senderUserId,required String text}) async { final value=text.trim(); if(value.isEmpty)throw ArgumentError('Message cannot be empty'); final d=await _read(); final cs=d['conversations'].map(_conversation).whereType<Conversation>().toList(); final index=cs.indexWhere((x)=>x.id==conversationId); if(index<0)throw StateError('Conversation was not found'); final c=cs[index]; if(c.userAId!=senderUserId&&c.userBId!=senderUserId)throw StateError('Sender is not in conversation'); await _authorized(c.connectionId,senderUserId); final timestamp=now().toUtc(); final m=Message(id:'message_${timestamp.microsecondsSinceEpoch}',conversationId:conversationId,senderUserId:senderUserId,text:value,sentAt:timestamp); cs[index]=Conversation(id:c.id,connectionId:c.connectionId,userAId:c.userAId,userBId:c.userBId,createdAt:c.createdAt,lastMessageAt:timestamp); d['conversations']=cs.map(_cj).toList(); d['messages']=[...d['messages'],_mj(m)]; await _write(d); return m; }
}
