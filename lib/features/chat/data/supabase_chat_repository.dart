...
class SupabaseChatRepository implements ChatRepository {
  ...
  @override Future<Message> sendMessage({required String conversationId,required String senderUserId,required String text}) async { _own(senderUserId); final value=text.trim(); if(value.isEmpty||value.length>2000) throw ArgumentError('Invalid message'); return _message((await client.rpc('send_message',params:{'p_conversation_id':conversationId,'p_body':value,'p_client_message_id':'${senderUserId}_${DateTime.now().microsecondsSinceEpoch}'})) as Map<String,dynamic>); }
}
