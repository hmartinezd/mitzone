...
class LocalConnectionRepository implements ConnectionRepository {
  ...
  ConnectionRequest? _tryRequest(Object? raw) {
    ...
    return ConnectionRequest(id: raw['id'] as String, senderUserId: raw['sender'] as String, recipientUserId: raw['recipient'] as String, encounterId: raw['encounter'] as String, createdAt: date, status: status);
  }
  ...
  @override Future<ConnectionRequest> sendRequest({required String senderUserId, required String recipientUserId, required String encounterId}) async {
    ...
    final existing = requests.where((r) => ((r.senderUserId == senderUserId && r.recipientUserId == recipientUserId) || (r.senderUserId == recipientUserId && r.recipientUserId == senderUserId)) && r.encounterId == encounterId).firstOrNull;
    if (existing != null) return existing;
    final request = ConnectionRequest(id: '$senderUserId-$recipientUserId-$encounterId', senderUserId: senderUserId, recipientUserId: recipientUserId, encounterId: encounterId, createdAt: now().toUtc(), status: ConnectionRequestStatus.pending);
  ...
  Future<ConnectionRequest> _change(String id, String user, ConnectionRequestStatus status) async {
    ...
    final old = requests[index]; final updated = ConnectionRequest(id: old.id, senderUserId: old.senderUserId, recipientUserId: old.recipientUserId, encounterId: old.encounterId, createdAt: old.createdAt, status: status); requests[index] = updated; data['requests'] = requests.map(_requestJson).toList();
