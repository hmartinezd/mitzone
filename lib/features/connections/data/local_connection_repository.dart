...
class LocalConnectionRepository implements ConnectionRepository {
  ...
  @override Future<List<ConnectionRequest>> getIncomingRequests(String id) async { final data = await _read(); return (data['requests'] as List).map((value) => _request(value as Map)).where((r) => r.recipientUserId == id && r.status == ConnectionRequestStatus.pending).toList(); }
  @override Future<List<ConnectionRequest>> getOutgoingRequests(String id) async { final data = await _read(); return (data['requests'] as List).map((value) => _request(value as Map)).where((r) => r.senderUserId == id).toList(); }
  ...
  @override Future<List<Connection>> getConnections(String id) async => ((await _read())['connections'] as List? ?? []).map((value) => _connection(value as Map)).where((c) => c.userAId == id || c.userBId == id).toList();
}
