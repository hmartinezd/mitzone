...
class SupabasePersonalityRepository implements PersonalityRepository {
  ...
  @override Future<PersonalityProfile?> getForUser(String userId) async { final row = await client.from('personality_profiles').select().eq('user_id', userId).maybeSingle(); return row == null ? null : PersonalityProfile.fromJson({...row, 'userId': row['user_id'], 'version': row['questionnaire_version'], 'completedAt': row['completed_at'], 'visibility': row['visibility']}); }
  ...
  @override Future<Map<String, double>> getCompatibilityWith(Set<String> ids) async { final result = <String, double>{}; for (final id in ids) { final row = await client.rpc('get_personality_compatibility', params: {'p_other_user_id': id}); if (row is Map && row['state'] == 'known' && row['value'] is num) result[id] = (row['value'] as num).toDouble(); } return result; }
}
