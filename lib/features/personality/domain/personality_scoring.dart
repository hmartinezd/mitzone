...
class PersonalityScoringService {
  ...
  PersonalityProfile? score({required String userId, required Map<String, int> answers, DateTime? completedAt}) {
    ...
    for (final trait in PersonalityTrait.values) {
      ...
      values[trait] = items.map((q) { final raw = (answers[q.id] ?? 1).clamp(1, 4); return (q.reverse ? 5 - raw : raw) / 4; }).reduce((a, b) => a + b) / items.length;
    }
    return PersonalityProfile(userId: userId, traits: values, version: 1, completedAt: completedAt ?? DateTime.now().toUtc());
  }
}
