...
class _PersonalityScreenState extends ConsumerState<PersonalityScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(
      title: const Text('Your social style'),
      actions: [
        TextButton(
    ...
    body: ListView(
      ...
      children: [
        const Text(
          'A few friendly questions can help you discover people with a similar social style. There are no right answers.',
        ),
        ...
        FilledButton(
          onPressed: answers.length == personalityQuestions.length
              ? _save
              : null,
  ...
  Future<void> _save() async {
    ...
    final profile = const PersonalityScoringService().score(
      userId: userId,
      answers: answers,
    )!;
    await ref.read(personalityRepositoryProvider).save(profile);
    ref.invalidate(currentPersonalityProvider);
