...
void main() {
  ...
  test(
    'participants can remove a connection and reconnect in the same context',
    () async {
      final request = await repository.sendRequest(
        senderUserId: 'a',
        recipientUserId: 'b',
      ...
      final fresh = await repository.sendRequest(
        senderUserId: 'a',
        recipientUserId: 'b',
        encounterId: 'encounter',
        contextId: 'event:a:b',
  ...
  test('removal rejects unrelated and missing connections', () async {
    ...
    final connection = (await repository.getConnections('a')).single;
    await expectLater(
      repository.removeConnection(connectionId: connection.id, userId: 'c'),
      throwsStateError,
    );
    await repository.removeConnection(connectionId: connection.id, userId: 'a');
    await expectLater(
      repository.removeConnection(connectionId: connection.id, userId: 'a'),
      throwsStateError,
    );
  });
